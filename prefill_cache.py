...
from model_nsql import NSQLTransformer, NSQLAttention, KVCache

class PrefillCacheContext:
    def __init__(self, context: str, context_token_size: int, model: NSQLTransformer):
...
class PrefillCache:
    ...
    def insert_with_nsql_transformer(self, key: str, model: NSQLTransformer):
        self.get(key)
        to_save = []
        for i, layer in enumerate(model.layers):
            attn: NSQLAttention = layer.attention
    ...
    def load_to_nsql_transformer(self, key: str, model: NSQLTransformer):
        lst = self.get(key)
        for i, layer in enumerate(model.layers):
            attn: NSQLAttention = layer.attention
            kv_cache: KVCache = attn.kv_cache
    ...
    def save(self):
        ...
        if ctx is None:
            return
        self.insert_with_nsql_transformer(ctx.context, ctx.model)

    def load(self):
        ctx = self.ctx
        if ctx is None:
            return
        self.load_to_nsql_transformer(ctx.context, ctx.model)
