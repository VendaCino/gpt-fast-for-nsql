...
def replace_linear_weight_only_int8_per_channel(module):
    for name, child in module.named_children():
        if isinstance(child, nn.Linear):
            bias = ['fc_in', 'fc_out', 'lm_head'].__contains__(name)
...
class WeightOnlyInt8Linear(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        o = (F.linear(input.to(dtype=torch.float32), self.weight.to(dtype=torch.float32)) * self.scales)
        if self.use_bias:
            o = o + self.bias
        return o.to(dtype=input.dtype)
        ...
        for fqn, mod in self.mod.named_modules():
            if isinstance(mod, torch.nn.Linear):
                ...
                weight_int4pack, scales_and_zeros = prepare_int4_weight_and_scales_and_zeros(
                    weight.to(torch.float16).to('cuda'), self.groupsize, self.inner_k_tiles
                )
