...
from model_nsql import NSQLTransformer
...
def logits_to_probs(logits, temperature: float = 1.0, top_k: Optional[int] = None, **sampling_kwargs):
...
def sample(logits, temperature: float = 1.0, top_k: Optional[int] = None, **sampling_kwargs):
...
def decode_n_tokens(model: Transformer, cur_token: torch.Tensor, input_pos: torch.Tensor, num_new_tokens: int,
                    callback=lambda _: _, eos: int = None, contains_eos=False, **sampling_kwargs):
    ...
    for i in range(num_new_tokens):
        ...
        if eos is not None and next_token.clone() == eos:
            if contains_eos:
        ...
        # print(f"decode one token use time : ${time.perf_counter() - t0}")
    return new_tokens, new_probs
...
def speculative_decode(
    ...
    **sampling_kwargs
    ...
    t0 = time.perf_counter()
    draft_tokens, draft_probs = decode_n_tokens(draft_model, cur_token.view(1, -1), orig_input_pos.clone(), speculate_k, eos=eos, contains_eos=True, **sampling_kwargs)
    ...
    if rejected_locations.shape[0] == 0: # All draft tokens have been accepted
        ...
        if last_token == eos or draft_last_token == eos:
            # print(f"model forward skip last eos")
...
def generate(
    ...
    **sampling_kwargs
    ...
    """
    prefill_cache: Optional[PrefillCache] = sampling_kwargs.get('prefill_cache')
...
def _load_model(checkpoint_path, device, precision, use_tp):
    with torch.device('meta'):
        if 'nsql' in checkpoint_path.parent.name:
            model = NSQLTransformer.from_name(checkpoint_path.parent.name)
        else:
...
def compile_this_file(is_speculative = False, use_tp = False, compile_prefill = False):
...
def main(
    ...
    if compile:
        compile_this_file(is_speculative, use_tp, compile_prefill)
